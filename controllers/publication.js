...
function prueba(req, res){
  res.status(200).send({ message: 'Publication Controller' });
}
...
function savePublication(req, res){
  ...
  if(!params.text) return res.status(200).send({ message: 'Debes publicar algo' });
...
// obtener publicaciones de usuarios que sigo
...
function getPublication(req, res){
    ...
    Publication.findById(publicationId, (err, publication) => {
      ...
      if(!publication) res.status(404).send({ message: 'No hay publicación' });
...
// subir archivo a la publicacion
      ...
      Publication.findOne({'user': req.user.sub, '_id':publicationId}).exec((err, publication) => {
        if(publication){
          Publication.findByIdAndUpdate(publicationId, {file: file_name}, {new:true}, (err, publicationUpdated) => {
            ...
            if(!publicationUpdated) return res.status(404).send({ message: 'No se ha podido cargar archivo' });
